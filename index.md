...
<section id="leadership">
  <div class="section-head">
    ...
    <span class="count">{{ site.data.leadership | size }}</span>
  </div>

  {% for entry in site.data.leadership %}
    <div class="exp{% if entry.status == 'current' %} current{% endif %}">
      <div class="exp-head">
        <div class="period">
          {{ entry.period }}
          {% if entry.status == 'completed' %}<span class="period-badge completed">Concluded</span>{% endif %}
          {% if entry.status == 'current' %}<span class="period-badge">Active</span>{% endif %}
        </div>
        <h3 class="role">{{ entry.role }}</h3>
        <div class="org">
          {% if entry.organization_url %}
            <a href="{{ entry.organization_url }}" target="_blank" rel="noopener">{{ entry.organization }}</a>
          {% else %}{{ entry.organization }}{% endif %}
        </div>
      </div>
      ...
      {% if entry.description %}
        <p class="exp-desc">{{ entry.description | markdownify | remove: '<p>' | remove: '</p>' }}</p>
      {% endif %}
      ...
      {% if entry.advisors or entry.regional_director or entry.sponsors %}
        <div class="exp-credits">
          {% if entry.advisors %}
            <div class="credit-block">
              <span class="credit-label">Faculty advisors</span>
              {% for a in entry.advisors %}{% if a.url %}<a href="{{ a.url }}" target="_blank" rel="noopener">{{ a.name }}</a>{% else %}{{ a.name }}{% endif %}{% unless forloop.last %} · {% endunless %}{% endfor %}
            </div>
          {% endif %}
          {% if entry.regional_director %}
            <div class="credit-block">
              <span class="credit-label">Regional Director</span>
              {% if entry.regional_director.url %}
                <a href="{{ entry.regional_director.url }}" target="_blank" rel="noopener">{{ entry.regional_director.name }}</a>
              {% else %}{{ entry.regional_director.name }}{% endif %}
            </div>
          {% endif %}
          {% if entry.sponsors %}
            <div class="credit-block">
              <span class="credit-label">Sponsors</span>
              {{ entry.sponsors | join: ' · ' }}
            </div>
          {% endif %}
        </div>
...
<section id="reviews">
  <div class="section-head">
    <h2>Peer Review</h2>
    <span class="count">{% assign total = 0 %}{% for r in site.data.reviews %}{% assign total = total | plus: r.count %}{% endfor %}{{ total }} reviews</span>
  </div>
  <div class="reviews-grid">
    {% for r in site.data.reviews %}
      <div class="review">
        <div class="review-journal">{{ r.journal }}</div>
        <div class="review-meta">
          {{ r.publisher }}{% if r.issn %} · ISSN {{ r.issn }}{% endif %}
        </div>
        <div class="review-count">
          <span class="review-num">{{ r.count }}</span>
          review{% if r.count != 1 %}s{% endif %} · {{ r.years }}
        </div>
      </div>
    {% endfor %}
  </div>
